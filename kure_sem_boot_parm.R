...
path_measerr = '
	...
	parm_abun_log ~ AFH1*bark_moist_pct.rao.ba + AFH2*wood_SG.rao.ba + AFH3*LogSeed.rao.ba +
...
write.csv(endfit_std, 'boot_parm.csv', row.names=F)
...
rownames(direct_rich)[rownames(direct_rich)=='parm_abun_log'] = 'abun_log' 
...
rownames(total)[rownames(total)=='parm_abun_log'] = 'abun_log' 
...
direct_abun = subset(endfit_ests, (lhs=='parm_abun_log')&(op=='~'))
...
# Save tables
write.csv(endfit_ests, 'Parm_testdata_parameterEstimates.csv')
write.csv(total, 'Parm_testdata_totaleffects.csv', row.names=T)
write.csv(direct_rich, 'Parm_testdata_directeffects_richness.csv', row.names=T)
write.csv(direct_abun, 'Parm_testdata_directeffects_abundance.csv', row.names=T)
write.csv(indirect, 'Parm_testdata_indirecteffects_via_abundance.csv', row.names=T)
write.csv(indirect_reg, 'Parm_testdata_indirecteffects_via_regS.csv', row.names=T)
write.csv(indirect_for, 'Parm_testdata_indirecteffects_via_forest.csv', row.names=T)


save.image('bootstrap_parm_richness_model.RData')
