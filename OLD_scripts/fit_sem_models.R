...
# Data includes squared versions of variables where univariate models suggest these are warranted.
# Note that richness and reg denote local and regional species richness of all species
# Change the file read in to use subsets of richness (e.g. Phys or Parm)
# These files generated by the fit_varpart_models.R script
use_data = read.csv('./Data/FIA lichen model data with sq vars.csv', row.names=1)
use_data = trans_data
...
# Define which variables have significant concave down univariate relationships
# note that this has not been done for ap or rh, and only for wetness and rain_lowRH
...
regvars = c('mat_reg_mean','iso_reg_mean','pseas_reg_mean','ap_reg_mean','rh_reg_mean',
	'mat_reg_cv','iso_reg_cv','pseas_reg_cv','ap_reg_cv','rh_reg_cv', 'regS_tree')
regPCA = prcomp(use_data[,regvars], center=T, scale=T)
...
RHclimvars = rownames(subset(use_preds, scale=='regional'&mode=='het'&type=='env'))
ROvars = rownames(subset(use_preds, scale=='regional'&mode=='opt'))

LHvars = rownames(subset(use_preds, scale=='local'&mode=='het'))
LOclimvars = rownames(subset(use_preds, scale=='local'&mode=='opt'&type=='env'))
LOclimvars = LOclimvars[-grep('soil', LOclimvars)] # Don't include soil
LOforvars = rownames(subset(use_preds, scale=='local'&mode=='opt'&type=='forest'))

# Fit full models subsets
LH_mod = glm.nb(richness~., data=fit_data[,c('richness', LHvars, paste(sq_vars[sq_vars %in% LHvars],2,sep=''))])
LO_clim_mod = glm.nb(richness~., data=fit_data[,c('richness', LOclimvars, paste(sq_vars[sq_vars %in% LOclimvars],2,sep=''))])
LO_for_mod = glm.nb(richness~., data=fit_data[,c('richness', LOforvars, paste(sq_vars[sq_vars %in% LOforvars],2,sep=''))])

RH_clim_mod = lm(reg~., data=fit_data[,c('reg', RHclimvars)]) # No squared terms in this model
RO_mod = lm(reg~., data=fit_data[,c('reg', ROvars, paste(sq_vars_reg[sq_vars_reg %in% ROvars],2,sep=''))]) # Don't need to add in sq_vars_reg since no longer including total_NS_r


# Dredge all model subsets
options(na.action = 'na.fail') # Prevents sub models from fitting to different data subsets

RH_clim_allmods = dredge(RH_clim_mod) # top 2 models worth looking at
use_RHclimmods = 1:2

RO_allmods = dredge(RO_mod, subset = dc(rain_lowRH_reg_mean, rain_lowRH_reg_mean2)) # best model by far includes all predictors
use_ROmods = 1

LH_allmods = dredge(LH_mod, subset = dc(propDead, propDead2) && dc(LogSeed.rao.ba, LogSeed.rao.ba2) 
	&& dc(wood_SG.rao.ba, wood_SG.rao.ba2) && dc(bark_moist_pct.rao.ba, bark_moist_pct.rao.ba2))

cum_weight = sapply(1:nrow(LH_allmods), function(i) sum(LH_allmods$weight[1:i]))
min(which(cum_weight>0.95))
LH_allmods[1:72,] # but really top 6 models look best
use_LHmods = 1:6

LO_clim_allmods = dredge(LO_clim_mod, subset = dc(light.mean, light.mean2) && dc(radiation, radiation2) && dc(wetness, wetness2))
use_LOclimmods = 1:3

LO_for_allmods = dredge(LO_for_mod, subset = dc(PC1, PC12) && dc(wood_SG.ba, wood_SG.ba2))
use_LOformods = 1:2

# Average models and keep variables with importance > 0.5
# I identified the top models I would use for averaging parameter estimates but it doesn't matter whether all models or top models are used to determin variable importances
RH_clim_avg = model.avg(RH_clim_allmods[use_RHclimmods,])
importance(RH_clim_avg)
keep_RHclim_vars = c('iso_reg_var','mat_reg_var','rain_lowRH_reg_var','wetness_reg_var')
RH_clim_avg = model.avg(RH_clim_allmods) # Using all models
importance(RH_clim_avg)

RO_avg = RO_allmods[use_ROmods,]# Only using best models, which has all variables
keep_RO_vars = c('iso_reg_mean','mat_reg_mean','pseas_reg_mean','rain_lowRH_reg_mean','wetness_reg_mean')
RO_avg = model.avg(RO_allmods) # Using all models
importance(RO_avg)

LH_avg = model.avg(LH_allmods[use_LHmods,])
importance(LH_avg)
keep_LH_vars = c('bark_moist_pct.rao.ba','diamDiversity','LogSeed.rao.ba','PIE.ba.tree','wood_SG.rao.ba','propDead','lightDist.mean')
LH_avg = model.avg(LH_allmods) # Using all models
importance(LH_avg)

LO_clim_avg = model.avg(LO_clim_allmods[use_LOclimmods,])
importance(LO_clim_avg)
keep_LOclim_vars = c('mat','pseas','radiation','rain_lowRH','wetness')
LO_clim_avg = model.avg(LO_clim_allmods) # Using all models
importance(LO_clim_avg)

LO_for_avg = model.avg(LO_for_allmods[use_LOformods,])
importance(LO_for_avg)
keep_LOfor_vars = c('bark_moist_pct.ba','bigTrees','light.mean','PC1','wood_SG.ba')
LO_for_avg = model.avg(LO_for_allmods) # Using all models
importance(LO_for_avg)

# Use PCA to determine which variables positively covary and create composite variables
# Do PCA with all data but only fit SEM to test data set

## Regional heterogeneity and optimality
## PCA of all regional variables that are important in GLMs
regPCA = prcomp(use_data[,c(keep_RO_vars, keep_RHclim_vars, 'regS_tree')], center=T, scale=T)
summary(regPCA); regPCA



# Heterogeneity composite variables are based on PCA of heterogeneity variables
# and combine variables that positively covary

RHclimPC = prcomp(use_data[,keep_RHclim_vars], center=T, scale=T)
summary(RHclimPC); RHclimPC
# All variables load positively on first PC so only need one to represent heterogeneity
# PC1 explains 70.2% of variance
RH_clim = predict(RHclimPC)[,'PC1']

RH_for = scale(use_data$regS_tree, center=T, scale=T)

LHPC = prcomp(use_data[,keep_LH_vars], center=T, scale=T)
summary(LHPC); LHPC
# diamDiversity and lightDist.mean load opposite to other variables
LHPC1 = prcomp(use_data[,keep_LH_vars[!(keep_LH_vars %in% c('lightDist.mean','diamDiversity'))]], center=T, scale=T)
LHPC2 = prcomp(use_data[,c('lightDist.mean','diamDiversity')], center=T, scale=T)
summary(LHPC1); LHPC1 # PC1 explains 54.4% of variance
summary(LHPC2); LHPC2 # PC1 explains 55.2% of variance

LH_1 = predict(LHPC1)[,'PC1']
LH_2 = predict(LHPC2)[,'PC1']

# Optimality composite variables are based on signs of coefficients from averaged best models
# This is because we don't know a priori in which direct each mean factor should affect richness
# So, combining them based on their innate variation in the data set could combine factors with opposite effects on richness
# Still having this problem- may want to actually use the coeficients to predict richness as the composite variable
# Or, even use coefs from models of abun ~ optimality
# But, then this would bias optimality over heterogeneity

ROvar_signs = ifelse(coef(RO_avg)[,keep_RO_vars]>0, 1, -1)
ROdata = sapply(1:length(keep_RO_vars), function(i) use_data[,keep_RO_vars[i]]*ROvar_signs[i])
colnames(ROdata) = keep_RO_vars
ROPC = prcomp(ROdata, center=T, scale=T)
summary(ROPC); ROPC
# PC1 explains 52.7% of variance

RO = predict(ROPC)[,'PC1']

LOclim_signs = ifelse(coef(LO_clim_avg)[keep_LOclim_vars]>0, 1, -1)
LOclimdata = sapply(1:length(keep_LOclim_vars), function(i) use_data[,keep_LOclim_vars[i]]*LOclim_signs[i])
colnames(LOclimdata) = keep_LOclim_vars
LOclimPC = prcomp(LOclimdata, center=T, scale=T)
summary(LOclimPC); LOclimPC
# PC1: increasing mat, 31.2% variance
# PC2: increasing wetness, 22.5% variance
LO_clim1 = predict(LOclimPC)[,'PC1']
LO_clim2 = predict(LOclimPC)[,'PC2']

LOfor_signs = ifelse(coef(LO_for_avg)[keep_LOfor_vars]>0, 1, -1)
LOfordata = sapply(1:length(keep_LOfor_vars), function(i) use_data[,keep_LOfor_vars[i]]*LOfor_signs[i])
colnames(LOfordata) = keep_LOfor_vars
LOforPC = prcomp(LOfordata, center=T, scale=T)
summary(LOforPC); LOforPC
# PC1: increasing bark moisture, but decreasing all others 
LO_for1 = predict(LOforPC)[,'PC1']
LO_for2 = predict(LOforPC)[,'PC2']


# Make a dataframe for the SEM (including scaled richness/abundance for use in sem
sem_data = data.frame(working_data[c('lichen.rich_log','tot_abun_log', 'regS')], use_data[,c('richness','reg','abun_log')], RH_for, RH_clim, RO, LH_1, LH_2, LO_clim1, LO_clim2, LO_for1, LO_for2)
write.csv(sem_data, './SEM models/pca_sem_data.csv', row.names=T)

sem_data_test = sem_data[testplots$yrplot.id,]
write.csv(sem_data_test, './SEM models/pca_sem_data_test.csv', row.names=T)
...
#################################################################
### Try fitting a piece-wise model
