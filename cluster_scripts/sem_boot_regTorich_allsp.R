...
regTorich_fit =  sem(path_regTorich, data=working_data_test, fixed.x=T, estimator='ML', se='robust.sem')
use_fit = regTorich_fit

mod_boot = bootstrapLavaan(use_fit, R=10000, FUN=function(x) c(parameterEstimates(x)$est,standardizedSolution(x)$est.std))
regTorich_boot = mod_boot

# Used to re-calculate tables outside of Kure
load('regToRich_AllSp_testdata_output.RData') 
mod_boot = regTorich_boot
use_fit = regTorich_fit 

# Save raw bootstrap output and models
response = 'AllSp'
modform = 'regToRich'
save(regTorich_fit, regTorich_boot, path_regTorich, file=paste(modform,response,'testdata_output.RData', sep='_'))
...
TP_P_mat = DE_mat[,'totalNS_reg'] + P_IE_pol_mat + IE_regS_mat[,'totalNS_reg']
colnames(TP_P_mat) = 'totalNS_reg'
TE_P_mat = matrix(DE_mat[,'totalNS_reg'] + IE_regS_mat[,'totalNS_reg'], nrow=nrow(stdmat))
...
# Pollution (via pollution's regional, local, and direct effects)
...
C_IE_P_mat_dir = stdmat[,use_ests]*DE_mat[,'totalNS_reg']
colnames(C_IE_P_mat_dir) = ests[use_ests,'rhs']
C_IE_P_mat = C_IE_P_mat_reg + C_IE_P_mat_loc + C_IE_P_mat_dir
...
IE_mat = cbind(IE_regS_mat, FH_IE_for_mat, P_IE_pol_mat, 
	C_IE_FH_mat, C_IE_cm_mat, C_IE_P_mat, C_IE_FH_mat_reg, C_IE_FH_mat_loc, C_IE_FH_mat_dir,
	C_IE_P_mat_reg, C_IE_P_mat_loc, C_IE_P_mat_dir)
...
indirect_reg$IEvar1 = c(rep('regS',ncol(IE_regS_mat)), rep('PIE.ba.tree', ncol(FH_IE_for_mat)), 
	...
	rep('regS_tree',3*ncol(C_IE_FH_mat)), rep('totalNS_reg', 3*ncol(C_IE_P_mat)))
indirect_reg$IEvar2 = c(rep(NA, ncol(IE_mat) - 3*(ncol(C_IE_FH_mat)+ncol(C_IE_P_mat))),
	rep(c('regS','loc','dir'),each=ncol(C_IE_FH_mat)), rep(c('regS','loc','dir'), each=ncol(C_IE_P_mat)))
...
TP_C_mat = sapply(Cvars, function(x){
	if(x %in% colnames(C_IE_P_mat)){
		DE_mat[,x] + IE_regS_mat[,x] + C_IE_FH_mat[,x] + C_IE_P_mat[,x] + C_IE_cm_mat[,x]
	} else {
		DE_mat[,x] + IE_regS_mat[,x] + C_IE_FH_mat[,x] + C_IE_cm_mat[,x]
	}
})
...
TE_C_mat = sapply(Cvars, function(x){
	if(x %in% colnames(C_IE_P_mat)){
		DE_mat[,x] + IE_regS_mat[,x] + C_IE_FH_mat_reg[,x] + C_IE_FH_mat_dir[,x] +
		C_IE_P_mat_reg[,x] + C_IE_P_mat_dir[,x]
	} else {
		DE_mat[,x] + IE_regS_mat[,x] + C_IE_FH_mat_reg[,x] + C_IE_FH_mat_dir[,x]
	}
})
...
# Make dataframe of total paths
TP_mat = cbind(TP_C_mat, TE_cm_mat, TE_f_mat, TP_FH_mat, TE_p_mat, TP_P_mat)
total = data.frame(predictor=colnames(TP_mat))
total$std.all = apply(TP_mat, 2, mean)
total$std.se = apply(TP_mat, 2, function(x) sqrt(var(x)))
total$std.ci.lower = apply(TP_mat, 2, function(x) quantile(x, p=0.025))
total$std.ci.upper = apply(TP_mat, 2, function(x) quantile(x, p=0.975))
