...
# Local variables
LH_for1 = prcomp(working_data_test[,c('propDead','PIE.ba.tree','bark_moist_pct.rao.ba','wood_SG.rao.ba')])
LH_for2 = prcomp(working_data_test[,c('diamDiversity','lightDist.mean')])


## Model with paths from regional variables to local richness
composite_mod = "

	# Define Local Optimality composite variables
	LO_clim =~ 0
	LO_clim ~ 1*wetness + rain_lowRH + mat + iso + pseas
	LO_clim ~~ 0*LO_clim

	# Define Local Heterogeneity composite variables
	LH_for =~ 0
	LH_for ~ 1*PIE.ba.tree + diamDiversity
	LH_for ~~ 0*LH_for

	# Define Regional Optimality composite variables
	#RO_clim ~ wetness_reg_mean + rain_lowRH_reg_mean 
	#RO_clim ~~ 0*RO_clim

	# Define Regional Heterogeneity composite variables
	#RH_clim ~ wetness_reg_var + rain_lowRH_reg_var
	#RH_clim ~~ 0*RH_clim

	# Effects of composites on abundance

	# Define latent variables for species richness
	S_L =~ lichen.rich_log
	S_R =~ regS
	
	# Effects of composites and regional richness on local richness
	#RO_clim + RH_clim =~ S_R
	S_L ~ LO_clim + LH_for + S_R
		

	# Estimate variance of species richness
	#regS ~~ regS
	lichen.rich_log ~~ lichen.rich_log
"
inspect(mod_fit, 'cov.lv')

mod_fit =  sem(composite_mod, data=working_data_test, fixed.x=T, std.lv=T, estimator='ML', se='robust.sem')
