...
save(use_data_test, O_mod, Ovars, sqdata, sq_vars, file='cluster_data_O.RData')

# Set up cluster for parallel computing
c1 = makeCluster(9, 'SOCK')
clusterEvalQ(c1, library(MASS))
clusterEvalQ(c1, load('cluster_data_O.RData'))


## Dredge models

Odredge = pdredge(O_mod, cluster=c1, beta=T, subset=dc('light.mean','light.mean2')&dc('wood_SG.ba','wood_SG.ba2')&dc('radiation','radiation2')&dc('PC1','PC12')&dc('wetness','wetness2')&dc('rain_lowRH_reg_mean','rain_lowRH_reg_mean2')&dc('wetness_reg_mean', 'wetness_reg_mean2'), m.min=2)


save(Odredge, file='local_rich_opt_model_dredge.RData')

stopCluster(c1)


# Subset to best models
best_weight = Odredge$weight[1] 
keep_models = which(Odredge$weight/best_weight>= 0.05)
Odredge_best = Odredge[keep_models,]

save(Odredge_best, file='local_rich_opt_models_best.Rdata')
