...
## Model with paths from regional variables to local richness and local -> regional
path_reverse = "
	...
	lichen_rich ~ r1fh1*bark_moist_pct.rao.ba + r1fh2*wood_SG.rao.ba + r1fh3*LogSeed.rao.ba +
		...
		r1cm1*wetness + r1cm2*rain_lowRH + r1cm3*iso + r1cm4*pseas + r1cm5*mat + r1cm6*radiation + 
...
#load('reverse_AllSp_testdata_output.RData') 
#mod_boot = reverse_boot
#use_fit = reverse_fit 

# Save raw bootstrap output and models
response = 'AllSp'
modform = 'reverse'
save(reverse_fit, reverse_boot, path_reverse, file=paste(modform,response,'testdata_output.RData', sep='_'))
...
use_ests = ests$lhs=='lichen_rich'&ests$op%in%c('~','~~')
...
direct_rich = direct_rich[direct_rich$predictor!='lichen_rich',]
