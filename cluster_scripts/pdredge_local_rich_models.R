...
save(use_data_test, L_mod, Lvars, sqdata, sq_vars, file='cluster_data_L.RData')

# Set up cluster for parallel computing
c1 = makeCluster(12, 'SOCK')
clusterEvalQ(c1, library(MASS))
clusterEvalQ(c1, load('cluster_data_L.RData'))


## Dredge models

Ldredge = pdredge(L_mod, cluster=c1, beta=T, subset=dc('light.mean','light.mean2')&dc('wood_SG.ba','wood_SG.ba2')&dc('radiation','radiation2')&dc('PC1','PC12')&dc('wetness','wetness2')&
	dc('propDead','propDead2')&dc('wood_SG.rao.ba','wood_SG.rao.ba2')&dc('bark_moist_pct.rao.ba','bark_moist_pct.rao.ba2'),
	m.min=2)


save(Ldredge, file='local_rich_model_dredge.RData')

stopCluster(c1)

# Subset to best models
best_weight = Ldredge$weight[1] 
keep_models = which(Ldredge$weight/best_weight>= 0.05)
Ldredge_best = Ldredge[keep_models,]

save(Ldredge_best, file='local_rich_models_best.Rdata')
