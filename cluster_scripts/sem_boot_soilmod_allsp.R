...
soilmod_fit =  sem(path_soilmod, data=working_soil_test, fixed.x=T, estimator='ML', se='robust.sem')
use_fit = soilmod_fit
...
load('soilmod_AllSp_testdata_output.RData') 
mod_boot = soilmod_boot
use_fit = soilmod_fit 

# Save raw bootstrap output and models
response = 'AllSp'
modform = 'soilmod'
save(soilmod_fit, soilmod_boot, path_soilmod, file=paste(modform,response,'testdata_output.RData', sep='_'))
