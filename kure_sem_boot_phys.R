...
path_measerr = '
	...
	# Latent variables
	lichen_rich =~ sqrt(0.75)*Phys_log
	...
	regPhys ~ R1*wetness + R2*rain_lowRH + R3*iso + R4*pseas + R5*mat
	...
	lichen_rich ~ FH1*bark_moist_pct.rao.ba + FH2*wood_SG.rao.ba + FH3*LogSeed.rao.ba +
		...
		C1*wetness + C2*rain_lowRH + C3*iso + C4*pseas + C5*mat + C6*radiation + R*regPhys + P*totalNS +
		A*phys_abun_log

	# Lichen abundance regression
	phys_abun_log ~ AFH1*bark_moist_pct.rao.ba + AFH2*wood_SG.rao.ba + AFH3*LogSeed.rao.ba +
...
write.csv(endfit_std, 'boot_phys.csv', row.names=F)
...
rownames(direct_rich)[rownames(direct_rich)=='regPhys'] = 'reg' 
rownames(direct_rich)[rownames(direct_rich)=='phys_abun_log'] = 'abun_log' 
...
rownames(total)[rownames(total)=='regPhys'] = 'reg' 
rownames(total)[rownames(total)=='phys_abun_log'] = 'abun_log' 
...
direct_abun = subset(endfit_ests, (lhs=='phys_abun_log')&(op=='~'))
...
# Save tables
write.csv(endfit_ests, 'Phys_testdata_parameterEstimates.csv')
write.csv(total, 'Phys_testdata_totaleffects.csv', row.names=T)
write.csv(direct_rich, 'Phys_testdata_directeffects_richness.csv', row.names=T)
write.csv(direct_abun, 'Phys_testdata_directeffects_abundance.csv', row.names=T)
write.csv(indirect, 'Phys_testdata_indirecteffects_via_abundance.csv', row.names=T)
write.csv(indirect_reg, 'Phys_testdata_indirecteffects_via_regS.csv', row.names=T)
write.csv(indirect_for, 'Phys_testdata_indirecteffects_via_forest.csv', row.names=T)

save.image('bootstrap_phys_richness_model.RData')
